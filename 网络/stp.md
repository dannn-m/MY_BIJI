...
## 4.选择指定端口
- 指定端口是一个网段上（两个交换机之间）的端口，它拥有**去往根桥**的[[内部根路径开销]]，也就是它具有接收发往根桥流量的最佳
...
### PortFast和BPDU防护
当交换机端口或交换机上电时，交换机端口会经历侦听和学习状态，每次要等延迟计时器过期，一共30s，此时DHCP进程可能会超时使ipv4客户端接收不到有效的ip地址
**注意**：icmpv6可能会发生这种情况，但路由器会继续发送icmpv6路由器通告消息·，让设备知道如何获取其地址信息
当交换机配置了PortFast时端口会立刻过度到转发状态，这项技术只用在接入端口上，连接到其他端口有形成环路风险
