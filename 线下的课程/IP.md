...
#### 7.路由优选规则
前置过滤 正式进入选路流程前，**先丢弃所有下一跳不可达的无效路由**，仅剩余的有效路由参与后续优先级比较。 
选路优先级（从高到低） 
1. **Preferred-Value（首选值）**：取值越大越优
2. **Local_Preference（本地优先级）**：取值越大越优 
3. **路由始发类型**：本地始发路由 优于 从其他对等体学习的路由 本地始发路由内部优先级：手动聚合 > 自动聚合 > network发布 > import引入 > 对等体学习 
4. **AS_Path 属性**：路径长度越短越优 
5. **Origin（起源属性）**：优先级从高到低为 `IGP` > `EGP` > `Incomplete` 
6. **MED（多出口鉴别）**：取值越小越优 
7. **邻居类型**：从 EBGP 对等体学到的路由 优于 从 IBGP 对等体学到的路由 
8. **下一跳 IGP 度量值**：去往 BGP 下一跳的 IGP 路由开销越小越优 > 关键说明：当前 8 条属性全部相同时，可形成 BGP 路由负载分担 
9. **Cluster_List（簇列表）**：列表长度越短越优（路由反射器场景生效） 
10. **Router ID / Originator_ID**：优选通告设备 Router ID（或 Originator_ID）更小的路由 
11. **对等体 IP 地址**：优选 IP 地址更小的对等体通告的路由
当前8条属性全部相同时可以形成负载分担
...
- 动态mac老化时间
- 静态mac学习
**注意**：对于mac安全可以在接口也可以在vlan视图下部署 静态 黑洞 老化时间是全局概念
## 端口安全
**种类**：
